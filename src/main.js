...
function getIcon(name) {
    ...
    if(n.includes('picture')) return 'image';
    return 'cube';
}
...
const ROOM_DATABASE = {
    ...
    officeroom: {
        ...
        items: [
            ...
            "Trashcan.glb", "Trashcan Small.glb", "Trash Bin.glb", "Bins.glb", "Lamp.glb", "Light Desk.glb", "Light Floor.glb", "Light Ceiling.glb", "Ceiling Light.glb", "Potted Plant.glb", "Houseplant.glb", "Plant - White Pot.glb", 
...
const camera = new THREE.PerspectiveCamera(50, window.innerWidth / window.innerHeight, 0.1, 1000); camera.position.set(0, 20, 30); 
...
renderer.domElement.setAttribute('tabindex', '1'); renderer.domElement.style.outline = 'none';
const container = document.getElementById('canvas-container'); if(container) { container.innerHTML = ''; container.appendChild(renderer.domElement); }

const dirLight = new THREE.DirectionalLight(0xffffff, 1.5); dirLight.position.set(20, 40, 20); dirLight.castShadow = true; 
dirLight.shadow.mapSize.set(4096, 4096); dirLight.shadow.camera.left = -50; dirLight.shadow.camera.right = 50; dirLight.shadow.camera.top = 50; dirLight.shadow.camera.bottom = -50;
scene.add(dirLight); scene.add(new THREE.AmbientLight(0xffffff, 0.8));

const orbit = new OrbitControls(camera, renderer.domElement);
const transformControl = new TransformControls(camera, renderer.domElement); transformControl.setMode('rotate'); transformControl.visible = false; transformControl.enabled = false;
...
function buildRoomShell(config) {
    ...
    roomMeshes.forEach(mesh => scene.remove(mesh)); roomMeshes = [];
    ...
    floor.rotation.x = -Math.PI / 2; floor.receiveShadow = true; scene.add(floor); roomMeshes.push(floor);
    if (config.wallColor) {
        const h = 6; const wallMat = new THREE.MeshStandardMaterial({ color: config.wallColor, side: THREE.DoubleSide });
        const createWall = (w, h, x, y, z, ry) => {
            const wall = new THREE.Mesh(new THREE.PlaneGeometry(w, h), wallMat); wall.position.set(x, y, z); if(ry) wall.rotation.y = ry; wall.receiveShadow = true; scene.add(wall); roomMeshes.push(wall);
        };
        createWall(config.width, h, 0, h/2, -config.depth/2, 0); createWall(config.depth, h, -config.width/2, h/2, 0, Math.PI/2); createWall(config.depth, h, config.width/2, h/2, 0, -Math.PI/2);
    }
}
...
function createObjectFromConfig(item, folderPath) {
    if (item.type === 'model' && item.fileName) {
        ...
        loader.load(fullPath, (gltf) => {
            ...
            if (item.isWallMounted) model.position.set(0, 2.0, -currentRoomConfig.depth/2 + 0.2); 
            else model.position.set(0, 0, 0); 
            model.traverse((child) => { if (child.isMesh) { child.castShadow = true; child.receiveShadow = true; } });
            model.userData = { type: 'model', name: item.name, path: fullPath, baseScale: s, isWallMounted: item.isWallMounted };
            scene.add(model); objects.push(model); limitObjectBounds(model); selectObject(model); addToHistory({ type: 'ADD', object: model });
        ...
        mesh.position.set(0, 0.5, 0); mesh.userData = { type: 'basic', name: item.name };
        scene.add(mesh); objects.push(mesh); limitObjectBounds(mesh); selectObject(mesh); addToHistory({ type: 'ADD', object: mesh });
    }
}

function selectObject(object) {
    if(object) {
        updatePropertiesPanel(object); if (transformControl.visible) transformControl.attach(object);
    } else {
...
renderer.domElement.addEventListener('pointerdown', (event) => {
    ...
    if (intersects.length > 0) {
        let target = intersects[0].object; while (target.parent && !objects.includes(target)) target = target.parent;
        if (objects.includes(target)) {
            if (transformControl.visible && transformControl.enabled) { transformControl.attach(target); updatePropertiesPanel(target); return; }
    ...
    selectObject(null); transformControl.detach(); transformControl.visible = false; transformControl.enabled = false;
});
...
renderer.domElement.addEventListener('pointermove', (event) => {
    ...
    if (raycaster.ray.intersectPlane(dragPlane, intersectPoint)) {
        draggedObject.position.x = intersectPoint.x - offset.x; draggedObject.position.z = intersectPoint.z - offset.z; limitObjectBounds(draggedObject);
    }
});

renderer.domElement.addEventListener('pointerup', () => {
    if (isDragging && draggedObject) { limitObjectBounds(draggedObject); addToHistory({ type: 'TRANSFORM', object: draggedObject }); }
    isDragging = false; draggedObject = null; orbit.enabled = true; 
});

window.addEventListener('keydown', (e) => {
    if (['INPUT', 'SELECT', 'TEXTAREA'].includes(document.activeElement.tagName)) return;
    const key = e.key.toLowerCase();
    if (key === 'delete' || key === 'backspace') { const obj = transformControl.object || objects.find(o => o.userData.name === document.getElementById('prop-type').innerText); if (obj) deleteObject(obj); }
    if (key === 'r') {
        if (transformControl.visible) { transformControl.detach(); transformControl.visible = false; transformControl.enabled = false; } 
        else { const objName = document.getElementById('prop-type').innerText; const target = objects.find(o => o.userData.name === objName); if (target) { transformControl.attach(target); transformControl.visible = true; transformControl.enabled = true; } }
    }
...
function deleteObject(obj) {
    if (!obj) return; addToHistory({ type: 'REMOVE', object: obj }); scene.remove(obj); objects.splice(objects.indexOf(obj), 1); selectObject(null); transformControl.detach();
}

function addToHistory(action) { history.push(action); redoStack.length = 0; }
function executeUndo() {
    if (history.length === 0) return; const action = history.pop(); redoStack.push(action);
    if (action.type === 'ADD') { scene.remove(action.object); objects.splice(objects.indexOf(action.object), 1); selectObject(null); } 
    else if (action.type === 'REMOVE') { scene.add(action.object); objects.push(action.object); selectObject(action.object); } 
    else if (action.type === 'TRANSFORM') { selectObject(action.object); }
}
...
function updateToolbar(roomKey, items) {
    ...
    items.forEach(item => {
        ...
        btn.className = 'tool-item'; btn.title = item.name; btn.innerHTML = `<i class="fa-solid fa-${item.icon}"></i>`;
...
function updatePropertiesPanel(object) {
    const propDetails = document.getElementById('prop-details'); const propContent = document.getElementById('prop-content');
    if (object) {
        if(propContent) propContent.classList.add('hidden'); if(propDetails) propDetails.classList.remove('hidden');
        document.getElementById('prop-type').innerText = object.userData.name || "Model"; document.getElementById('prop-x').innerText = object.position.x.toFixed(2); document.getElementById('prop-z').innerText = object.position.z.toFixed(2);
    } else {
        if(propContent) propContent.classList.remove('hidden'); if(propDetails) propDetails.classList.add('hidden');
    }
}
...
if (roomSelector) {
    roomSelector.addEventListener('change', (e) => {
        const roomKey = e.target.value; const roomConfig = ROOM_DATABASE[roomKey];
        if (roomConfig) {
            buildRoomShell(roomConfig); updateToolbar(roomKey, roomConfig.items);
            objects.forEach(obj => scene.remove(obj)); objects.length = 0; selectObject(null); transformControl.detach();
            camera.position.set(0, 20, 30); camera.lookAt(0, 0, 0); renderer.domElement.focus();
        }
    });
}
...
function checkAuth() {
    if(currentUser) {
        ...
        const welcome = document.getElementById('studio-welcome'); if(welcome) { welcome.classList.remove('hidden'); welcome.style.opacity = '1'; }
    } else {
...
if(submitBtn) submitBtn.addEventListener('click', async () => {
    const email = document.getElementById('email-input').value.trim(); const pass = document.getElementById('pass-input').value;
...
if(btnSave) btnSave.addEventListener('click', () => {
    ...
    if(name) {
        const data = objects.map(o => ({ type: o.userData.type, name: o.userData.name, path: o.userData.path, baseScale: o.userData.baseScale, position: o.position, rotation: o.rotation, scale: o.scale, isWallMounted: o.userData.isWallMounted }));
...
if(btnLoad) btnLoad.addEventListener('click', async () => {
    ...
    toggleModal(true);
    const list = document.getElementById('save-list'); list.innerHTML = 'Đang tải...';
    const data = await loadDesignFromCloud(currentUser.uid); list.innerHTML = '';
    if(data) {
        ...
        div.onclick = () => {
            ...
            toggleModal(false); alert("Đã tải!");
        };
